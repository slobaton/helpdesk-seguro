<div class="grid grid-cols-1 md:grid-cols-3 gap-6"
    ...
    <div class="p-4 bg-white dark:bg-gray-900 rounded-xl shadow">
        <h3 class="font-semibold mb-3">Open</h3>
        <div id="kanban-open" class="space-y-3 min-h-[200px] p-2 border rounded bg-gray-50 dark:bg-gray-800"
            ...
            @foreach ($open as $t)
                <div class="p-3 bg-white dark:bg-gray-700 rounded shadow-sm"
                    ...
                    <a href="{{ route('tickets.show', $t['id']) }}" class="text-xs underline">Open</a>
                </div>
            @endforeach
        </div>
    </div>
    ...
    {{-- Column: In Progress --}}
    <div class="p-4 bg-white dark:bg-gray-900 rounded-xl shadow">
        <h3 class="font-semibold mb-3">In Progress</h3>
    ...
    {{-- Column: Closed --}}
    <div class="p-4 bg-white dark:bg-gray-900 rounded-xl shadow">
        <h3 class="font-semibold mb-3">Closed</h3>
        <div id="kanban-closed" class="space-y-3 min-h-[200px] p-2 border rounded bg-gray-50 dark:bg-gray-800"
            ...
            @foreach ($closed as $t)
                <div class="p-3 bg-white dark:bg-gray-700 rounded shadow-sm"
                     data-ticket-id="{{ $t['id'] }}">
                    <div class="text-sm font-semibold">{{ $t['title'] }}</div>
                    <div class="text-xs opacity-70">Resolved at: {{ $t['resolved_at'] ?? '—' }}</div>
                    <a href="{{ route('tickets.show', $t['id']) }}" class="text-xs underline">Open</a>
                </div>
            @endforeach
        </div>
