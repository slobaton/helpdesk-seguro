...
<nav x-data="{ open: false }" class="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-700">
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                ...
                <div class="mr-4 hidden md:flex items-center gap-1">
                    @foreach (auth()->user()->getRoleNames() as $role)
                        <span class="text-xs px-2 py-1 rounded-full bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-100">
                            {{ $role }}
                        </span>
                    @endforeach
                </div>
                    ...
                    <x-slot name="trigger">
                        <button
                            class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4
                                   ...
                                   hover:text-gray-700 dark:hover:text-gray-200 focus:outline-none transition ease-in-out duration-150">
    ...
    <div :class="{ 'block': open, 'hidden': ! open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @hasanyrole('Admin|Technician')
            <x-responsive-nav-link :href="route('tickets.board')" :active="request()->routeIs('tickets.board')">
                {{ __('Kanban Board') }}
            </x-responsive-nav-link>
            @endhasanyrole
        </div>
